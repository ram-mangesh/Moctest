...
const HeartRateChart = ({ data = [], width = 700, height = 220 }) => {
  ...
  if (!data || !data.length) {
    return (
      <div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', fontSize: '13px' }}>
        No heart rate data detected in current range.
      </div>
    );
  }
  ...
  const step = spread > 20 ? Math.ceil(spread / 5) : 5;
  ...
  return (
    <div style={{ width: '100%', overflowX: 'auto' }}>
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto' }}>
        ...
        {/* Zone bands */}
        <rect x={padding.left} y={getY(100)} width={chartW} height={Math.max(0, getY(60) - getY(100))} fill="#10b981" opacity={0.05} rx={4} />
        {maxVal > 140 && <rect x={padding.left} y={getY(140)} width={chartW} height={Math.max(0, getY(100) - getY(140))} fill="#f59e0b" opacity={0.05} rx={4} />}
